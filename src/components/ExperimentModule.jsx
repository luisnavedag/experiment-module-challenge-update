...
const ExperimentModule = ({ data, onChange }) => {
  const [open, setOpen] = useState(false)
  const [openForm, setOpenForm] = useState(false)

  const [status, setStatus] = useState(data.status || 'unlocked')

  const [iterations, setIterations] = useState(data.iterations || [])
  const [newTitle, setNewTitle] = useState('')

  useEffect(() => {
    onChange({ iterations, status })
  }, [iterations, status])
  ...
  return (
    <div className='border rounded-md p-4 flex flex-col w-full bg-gray-50'>
      <div
        ...
        onClick={() => setOpen(!open && !(status === 'locked'))}
      >
        <h1 className='text-xl font-bold'>Experiment Module</h1>
        <div>{status === 'locked' ? <LockIcon /> : <LockOpenIcon />}</div>
      </div>
